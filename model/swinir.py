...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=48, patch_size=1, in_chans=3,
                 ...
                 **kwargs):
        super(SwinIR, self).__init__()  # 'pixelshuffledirect'
        upscale = 4
        img_size = 32 # img_size // upscale 48
...
if __name__ == '__main__':
    # model = HAT().cuda()
    height = 48 #int((1024 // 4 // 8 + 1) * 8)
    width = 48 #int((720 // 4 // 8 + 1) * 8)
    x = torch.randn((1, 3, height, width)).cuda()
    model = SwinIR(img_size=(height, width)).cuda()
