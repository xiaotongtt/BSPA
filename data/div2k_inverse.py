...
class DIV2K_INVERSE(srdata.SRData):
    def __init__(self, args, train=True):
        super(DIV2K_INVERSE, self).__init__(args, train)
        self.repeat = args.test_every // (args.n_train // args.batch_size)#20
        self.args = args
    ...
    def _scan(self):
        ...
        # for id in range(1, 11): #sorted(os.listdir(self.class_path)):
            ...
            for data_name in os.listdir(os.path.join(self.class_path, class_id)  + '/LR'):  # + '/old'
                ...
                hr_filepath = os.path.join(os.path.join(self.class_path, class_id)  + '/HR', data_name.replace('x'+str(self.args.scale[0]), ''))  # + '/old'
                i += 1
            #print(lr_filepath)
    ...
    def _set_filesystem(self, dir_data):
        ...
        self.ext = '.png'
        self.class_path = self.apath + '/subimages_classes_mse_' + self.args.un_factor +'_group10' #+ '_x' + str(self.args.scale[0])
    ...
    def __len__(self):
        if self.train:
            ...
            return len(self.images_hr)  # [0:1600]
        else:
    ...
    def _get_index(self, idx):
        if self.train:
            #rint(idx)
            return idx % len(self.images_hr)  # images_hr_normal
        else:
            return idx
    ...
    def __getitem__(self, idx):
        ...
        ###反转采样
        # meta = dict()
        lr_sample, hr_sample, filename = self._load_file(sample_index)  # , "inverse"
        ...
        #print(idx, end='\t')
        # print("hhhhh", e - s)
        return lr_sample_tensor, hr_sample_tensor, filename, self.idx_scale